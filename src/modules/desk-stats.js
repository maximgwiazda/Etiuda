...
/* The bucket for one day, made on first use. `daysSince` is the earliest day this desk holds a
   bucket for, which is what a span answer tells the reader it can speak for. */
function statsDay(pack, at){
  ...
  if(!b||typeof b!=="object"){
    b=pack.days[day]={cards:{},intents:{},misses:0,langs:{}};
...
function statsDoc(pack, info){
  const from=String(info&&info.period&&info.period.from||""), to=String(info&&info.period&&info.period.to||"");
  ...
  if(spanned){
    ...
    Object.keys(days).filter(d=>STATS_YMD.test(d)&&d>=from&&d<=to).sort().forEach(d=>{
      const b=days[d]||{};
      Object.keys(b.cards||{}).forEach(id=>{ const n=b.cards[id]|0; if(n){ uc[id]=(uc[id]|0)+n; at[id]=d; } });
      Object.keys(b.intents||{}).forEach(id=>{ ic[id]=(ic[id]|0)+(b.intents[id]|0); });
      Object.keys(b.langs||{}).forEach(c=>{ lc[c]=(lc[c]|0)+(b.langs[c]|0); });
      misses+=b.misses|0;
    });
