...
function applyCardColumns(){
  ...
  colTries=0;
  let n=colCount();
  colLastN=n;                        // what the WIDTH allows, which is what a resize compares
  if(n<2) return;
...
/* A rescue that must also survive the NEXT boot: applied here, before anything glass is
   drawn, and readable in the panel above. */
...
function activateCatalog(c,opts){
  ...
  try{ clearTimeout(tabSaveTimer); ssDel(TAB_KEY); eWiping=true; }catch(e){}
  location.reload();
  return true;
}
...
addEventListener("resize",()=>{
  ...
  if(tourRunning) scheduleTourPlace();                  // spotlight follows its target
