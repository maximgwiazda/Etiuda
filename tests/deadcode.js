/* Dead-code scan, beside test.js and i18n-scan.js. Reads engine/etiuda.html through engine.js.
 *
 * Finds top-level functions and consts that nothing else in the file mentions. It counts
 ...
 *
 *   node deadcode.js            names nothing references
 *   node deadcode.js --once     names referenced exactly once (the declaration plus one use)
 *
...
const names = [];
masked.replace(/^function\s+([A-Za-z_$][\w$]*)\s*\(/gm, (m, n) => { names.push([n, "function"]); return m; });
masked.replace(/^const\s+([A-Za-z_$][\w$]*)\s*=/gm, (m, n) => { names.push([n, "const"]); return m; });

const rows = [];
names.forEach(([n, kind]) => {
  ...
  rows.push({ n, kind, uses, viaString });
});
...
show.sort((a, b) => a.n.localeCompare(b.n)).forEach(r => {
  const line = SRC.slice(0, SRC.indexOf((r.kind === "function" ? "function " : "const ") + r.n)).split("\n").length;
  console.log("  " + r.kind.padEnd(9) + r.n.padEnd(28) + "line " + line);
});
